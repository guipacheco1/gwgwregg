...
def norm_uf(v: str) -> str:
    n = normaliza(v)
    return _UFS.get(n, n)


def norm_periodo(v: str) -> str:
    """Leva formatos comuns para AAAAMM: 20240301, 202403, 2024-03-01, 01/03/2024, 03/2024, 2024/03."""
    s = str(v).strip()
    if not s:
        return s
    if re.fullmatch(r"\d+(\.0+)?", s):  # numérico, às vezes vem como 20240301.0
        s = s.split(".")[0]
    ...
    if len(p) >= 2:
        anos = [x for x in p if len(x) == 4]
        if anos:
            ano = anos[0]
            resto = [x for x in p if x is not ano]
            if len(p) == 3 and len(p[0]) == 4:      # AAAA-MM-DD
                mes = p[1]
            elif len(p) == 3:                       # DD/MM/AAAA
                mes = p[1]
            else:                                   # MM/AAAA ou AAAA/MM
                mes = resto[0]
...
NORMALIZADORES = {
    "uf": norm_uf, "periodo": norm_periodo, "cidade": normaliza, "regiao": normaliza, "canal": normaliza,
    "mercado": normaliza, "grupo": normaliza, "produto": normaliza,
}


def _padroniza(df: pd.DataFrame, mapa: dict) -> pd.DataFrame:
    """Converte números e padroniza as chaves (maiúsculas, sem acento, UF em sigla, período em AAAAMM),
    para que 'Clínicas' case com 'CLINICAS', 'São Paulo' com 'SP' e '2024-03' com '20240301'."""
    ...
    for c in ("apresentacao", "pdv"):
        if c in out:
            out[c] = out[c].fillna("").astype(str).str.strip()
    for c, fn in NORMALIZADORES.items():
        if c in out:
            v = out[c].fillna("").astype(str)
            uniq = v.unique()
            out[c] = v.map(dict(zip(uniq, map(fn, uniq))))
    return out


def _chaves_base(tp: pd.DataFrame, tm: pd.DataFrame) -> list[str]:
    """Colunas que definem o loop, presentes nos dois arquivos.
    Produto/marca só entra se os valores dos dois arquivos casarem (ao menos metade das unidades)."""
    chaves = [c for c in ("periodo", "mercado", "grupo", "canal") if c in tp and c in tm]
    if "produto" in tp and "produto" in tm and tp["un"].sum() > 0:
        casa = tp["produto"].isin(set(tm["produto"]))
        if tp.loc[casa, "un"].sum() >= 0.5 * tp["un"].sum():
...
def preparar(pdv_raw, pdv_map, merc_raw, merc_map, cfg: Config):
    ...
    for campo, sel in (("periodo", cfg.periodos), ("mercado", cfg.mercados), ("grupo", cfg.grupos),
        ...
        if sel:
            sel = {NORMALIZADORES.get(campo, str)(x) for x in sel}
            if campo in tp:
...
def _metricas(X, pdvs, prods, cfg, status):
    ...
    un_pdv, un_merc = float(pdvs["un"].sum()), float(R.sum())
    motivos = []
    if abs(un_pdv - un_merc) > 0.5:
        motivos.append("Unidades dos PDVs diferentes do mercado")
    elif dev_merc >= 0.5:
        motivos.append("Mix de apresentações não fecha com o mercado")
    if cfg.usar_valor and ok_val < 1:
        motivos.append("Valor (PF) fora da tolerância")
    if cfg.usar_primo and ok_pri < 1:
        motivos.append("Número primo não bate")
    return {
        "status": status,
        "qualidade": "Aproximado" if motivos else "Exato",
        "motivo": " + ".join(motivos),
        "cobertura_pdv_vs_mercado": un_pdv / un_merc if un_merc else None,
...
def executar(pdv_raw, pdv_map, merc_raw, merc_map, cfg: Config,
    ...
    aloc = pd.concat(alocs, ignore_index=True) if alocs else pd.DataFrame(
        columns=["_id", "apresentacao_estimada", "un_estimada", "fator_un", "preco_medio", "nivel", "_chave"])
